 ...
 */
const _            = require('underscore-mixins2'),
    prepare        = require('oracle-prepare-sql'),
    async          = require('async'),
    inherits       = require('util').inherits,
    AbstractDriver = require('../AbstractDriver'),
    requiredVer    = '~1.9.3'; // oracledb@1.9.3 >= requiredVer < oracledb@1.10.0
 ...
 */
OracleDB.prototype.getDbConnection = function() {
    ...
    console.log(par, 'par w getDbConnection');
    ...
    console.log('przed dbPool.getConnection, poolUsage', poolUsage);

    //console.log('dbPool', dbPool);

    dbPool.getConnection(function(err, connection) {
        console.log('w callbacku dbPool.getConnection err=', err);
        if (err) {
            if(err.message.match(/ORA-24418/)) {
                if (par.probes < self.getCfg().getConnMaxProbes) {
                    ...
                    let debug = { timeout: true, probes: par.probes };
 ...
 */
OracleDB.prototype.executeTransaction = function() {
 ...
 */
OracleDB.prototype.selectOneRowSql = function() {
 ...
 */
OracleDB.prototype.selectOneRow = function() {
 ...
 * @params 'sql:string, bind:object|Array, [opt:object], cb:function'
 */
OracleDB.prototype.selectOneValueSql = function() {
 ...
 */
OracleDB.prototype.selectOneValue = function() {
 ...
 * @params 'sql:string, bind:object|Array, [opt:object], cb:function'
 */
OracleDB.prototype.selectAllRowsSql = function() {
    ...
    //console.log(par, 'par przed this.getDbConnection');

    this.getDbConnection(function(err, connection) {
        console.log('w callbacku getDbConnection');
        if (err) {
            par.cb(new MyError(err));
            return;
        }

        console.log('mam connection');
 ...
 */
OracleDB.prototype.selectAllRows = function() {
 ...
 */
OracleDB.prototype.runProcedure = function() {
    ...
    if(_.lookup(par.opt, 'dbmsOutput')) { // when we catch DBMS_OUTPUT.PUT_LINE
        async.waterfall(
            [
                ...
                releaseDbConnection,
                function(data) {
                    par.cb(null, _.lookup(data, 'results.outBinds'), _.lookup(data, 'dbmsOutput'));
                }
            ],
        ...
        async.waterfall(
            [
                ...
                releaseDbConnection,
                function(data) {
                    par.cb(null, _.lookup(data, 'outBinds') || []);
                }
            ],
            getCatchErrorFn(par.cb)
        );
 ...
 */
OracleDB.prototype.insertReturningId = function() {
 ...
 */
OracleDB.prototype.insertReturningIdSql = function() {
 ...
 * @params 'sql:string, [bind:object|Array], [opt:object], cb:function'
 */
OracleDB.prototype.querySql = function() {
    ...
    this.getDbConnection(function(err, connection) {
        ...
        connection.execute(par.sql, par.bind, { isAutoCommit: true, autoCommit: true }, function(err, result) { // isAutoCommit for oracledb < 0.5
            console.log('oracledb.js querySql connection.execute callback err=', err);
            if (err) {
                connection.release(function(err) {
                    console.log('oracledb.js querySql connection.release err=', err);
                }); // ignore error
                par.cb(new MyError(err, {sql: par.sql, bind: par.bind}));
                return;
            }
 ...
 */
OracleDB.prototype.update = function() {
 ...
 */
OracleDB.prototype.insert = function() {
 ...
 */
OracleDB.prototype.del = function() {
 ...
 * @params 'sql:string, bind:object|Array, [opt:object], cb:function'
 */
OracleDB.prototype.selectClobValueSql = function() {
...
function invokeNextProbeAfterRandomDelay(cb, probes, waitTime, drvObj) {
    const delay = getRandomConnWaitTime(drvObj.getCfg());
    probes++;
