...
function dalFactory(dbDriverName, cfg, cb) {
    ...
    return new Promise((resolve, reject) => {
        if(typeof dbDriverName !== 'string') {
            reject(new Error('First parameter of dalFactory should by DB driver name'));
        }
        if(typeof cfg !== 'object') {
            reject(new Error('Second parameter of dalFactory should by configuration object for DB driver'));
        }
        ...
        if(!pkgJson) {
            reject(new Error('package.json of ' + dbDriverName + ' is missing'));
        }
        if(!pkgJson.version) {
            reject(new Error('No version specified for ' + dbDriverName + ' in package.json'));
        }
