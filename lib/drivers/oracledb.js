...
const
    ...
    requiredVer    = '~1.12.0'; // oracledb@1.10.0 >= requiredVer < oracledb@1.11.0
...
const lib = {
    ...
    setupPool: function(cb) {
        ...
        this.getDriver().createPool(this.getCfg().connection, (err, pool) => {
            if (err) {
                cb(new Error(err));
                return;
            }
    ...
    selectOneRowSql: function() {
        const
            par     = this.getFnParams(arguments, 'selectOneRowSql'),
            execOpt = {};
        ...
        this.getDbConnection((err, connection) => {
            ...
            connection.execute(par.sql, par.bind, execOpt, (err, result) => {
                if (err) {
                    connection.release(getNoopCallback('selectOneRowSql execute err'));
                ...
                if(!result || !result.rows) {
                    par.cb(null, []);
                    connection.release(getNoopCallback('selectOneRowSql result 1'));
                    return;
                }

                if(result.rows.length > 1) {
                    par.cb(new MyError('Wrong number of rows returned from database (' + result.rows.length + ')'));
                    connection.release(getNoopCallback('selectOneRowSql rows.length > 1'));
                    return;
                }

                if(!_.get(par, 'opt.fetchClobs', false)) {
                    par.cb(null, result.rows[0]);
                    connection.release(getNoopCallback('selectOneRowSql result 2'));
                    return;
                }
                ...
                _.each(row, (lob, field) => {
                    if(lob && typeof lob.iLob === 'object') {
                        lobsToProcess.push({ field, lob });
                    }
                });

                async.map(lobsToProcess, fetchLobData, (err, results) => {
                    _.each(results, v => {
                        row[v.field] = v.outData;
                    });

                    par.cb(null, row);
                    connection.release(getNoopCallback('selectOneRowSql result 3'));
                });
    ...
    selectOneValueSql: function() {
        ...
        this.getDbConnection((err, connection) => {
            ...
            connection.execute(par.sql, par.bind, { outFormat: ARRAY }, (err, result) => {
                if (err) {
                    connection.release(getNoopCallback('selectOneValueSql execute'));
                    par.cb(new MyError(err, {sql: par.sql, bind: par.bind}));
                    return;
                }
    ...
    selectAllRowsSql: function() {
        ...
        this.getDbConnection((err, connection) => {
            ...
            connection.execute(par.sql, par.bind, execOpt, (err, result) => {
                if (err) {
                    connection.release(getNoopCallback('selectAllRowsSql execute'));
                    par.cb(new MyError(err, { sql: par.sql, bind: par.bind }));
                    return;
                }

                fetchResultSet(result, connection, par, (err, rows) => {
                    if(err) {
                        par.cb(err);
                        return;
                    }
        ...
        if(_.get(par.opt, 'dbmsOutput')) { // when we catch DBMS_OUTPUT.PUT_LINE
            async.waterfall(
                [
                    ...
                    (data) => {
                        let out = {};
                        _.assign(out, { dbmsOutput: _.get(data, 'dbmsOutput') }, _.get(data, 'results.outBinds'));
                        par.cb(null, out, _.get(data, 'dbmsOutput'));
                    }
                ],
    ...
    querySql: function() {
        ...
        this.getDbConnection((err, connection) => {
            ...
            connection.execute(par.sql, par.bind, execOpt, (err, result) => {
                if (err) {
                    connection.release(getNoopCallback('querySql execute'));
                    par.cb(new MyError(err, {sql: par.sql, bind: par.bind}));
                    return;
                }
     ...
     * @params 'sql:string, bind:object|Array, [opt:object], cb:function'
     */
    selectOneClobValueSql: function() {
        const
 ...
 */
lib.selectClobValueSql = lib.selectOneClobValueSql;
...
function setNlsSessionParams(conn, params, callback) {
    ...
    async.mapSeries(sqls, executeSql, err => {
        if (err) {
