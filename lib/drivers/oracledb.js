...
const
    ...
    requiredVer    = '~5.1.0'; // oracledb@5.1.0 >= requiredVer < oracledb@5.2.0
...
class OracleDB extends AbstractDriver {
    ...
    constructor(driver, cfg) {
        ...
        // super not set all needed configs
        if(cfg.driverOpts) {
            Object.keys(cfg.driverOpts).forEach(key => {
                driver[key] = cfg.driverOpts[key];
            });
        }
        ...
        this.OUT_FORMAT_ARRAY = driver.OUT_FORMAT_ARRAY;
        this.OUT_FORMAT_OBJECT = driver.OUT_FORMAT_OBJECT;
    ...
    selectOneValue() {
        ...
        return this.selectOneValueSql(obj.sql, obj.params, par.opt, par.cb);
    }
    ...
    selectAllRowsSql() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.getDbConnection(par.opt)
                ...
                .catch(err => {
                    if(_conn !== null && _conn.autoCommit) {
                ...
                async.waterfall(
                    [
                        ...
                        (conn, cb) => {
                            let dbmsOutput = [];
                            try {
                                conn.execute(sql, binds, { autoCommit: conn.autoCommit }, (err, result) => {
                                    if (err) {
    ...
    insertReturningId() {
        ...
        return this.insertReturningIdSql(obj.sql, obj.params, par.sequence, par.opt, par.cb);
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.getDbConnection(par.opt)
                ...
                .then(result => {
                    ...
                    debugCtx = { par };
                    return _conn.execute(par.sql, par.bind, { autoCommit: _conn.autoCommit });
                })
                .then(result => {
                    _result = result;
                    if (_conn.autoCommit) {
        ...
        return new Promise((resolve, reject) => {
            this.getDbConnection(par.opt)
                .then(connection => {
                    ...
                    par.bind = par.bind || [];
                    debugCtx = { sql: par.sql, bind: par.bind };
                    execOpt.autoCommit = _conn.autoCommit;
    ...
    update() {
        ...
        return this.querySql(obj.sql, obj.params, par.opt, par.cb);
    }
    ...
    selectOneClobValue() {
        ...
        return this.selectOneClobValueSql(obj.sql, obj.params, par.opt, par.cb);
    }
...
function getOutFormatDriverIdentifier(outFormat, driver) {
    switch (outFormat) {
        ...
        case 'out_format_array':
            return driver.OUT_FORMAT_ARRAY;
        case 'out_format_object':
            return driver.OUT_FORMAT_OBJECT;
    }
    throw new Error(`Value ${outFormat} is not supported for config key: outFormat (supported values: array, object, out_format_array, out_format_object)!`);
}
        ...
        result.resultSet.getRows(numRows, (err, rows) => {
            ...
            _.each(rows, (row) => {
                _.each(row, (lob, field) => {
                    if(lob && typeof lob === 'object' && lob.type === 2017) {
                        lob.getData().then(data => {
                            row[field] = data;
                        });
                    }
                });
